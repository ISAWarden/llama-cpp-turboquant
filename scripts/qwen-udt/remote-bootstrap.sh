...
#   export REPO_URL=https://github.com/ISAWarden/llama-cpp-turboquant.git
#   export REPO_BRANCH=master
#   export DEST=$HOME/llama-cpp-turboquant
...
REPO_URL="${REPO_URL:-https://github.com/ISAWarden/llama-cpp-turboquant.git}"
...
if [[ -f "$(pwd)/CMakeLists.txt" ]]; then
  ...
  DEST="${DEST:-$HOME/llama-cpp-turboquant}"
  if [[ ! -d "$DEST/.git" ]]; then
