...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    add_opt(common_arg(
        ...
        string_format("alias for --checkpoint-min-step (default: %d, 0 = no minimum)", params.checkpoint_min_step),
        [](common_params & params, int value) {
            if (value < 0) {
                throw std::invalid_argument("checkpoint-every-n-tokens must be non-negative");
            }
            params.checkpoint_min_step = value;
        }
    ...
    add_opt(common_arg(
        {"--moe-expert-boost", "--moe-expert-cache"}, "N",
        string_format("device-resident MoE expert slice cache budget in MiB (0 = disabled, default: %d)", params.moe_expert_boost),
        [](common_params & params, int value) {
            if (value < 0) {
                throw std::invalid_argument("invalid value");
            }
            params.moe_expert_boost = value;
        }
    ).set_env("LLAMA_ARG_MOE_EXPERT_BOOST"));
